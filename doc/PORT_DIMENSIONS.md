...
| **父子状态边**（父读子状态）的静态检查 | ✅ 2026-09-24 补齐：`ChildState`（父声明读子节点的哪些状态）对 `State`（子声明发布哪些状态），`state_declarations_agree<Parent,Child>()` 按**名字+顺序+量纲**检查；两条边可分别断言。**2026-09-24 起已推广到任意分叉树**：父的扁平声明必须等于各子声明**按孩子顺序的拼接**（`children_references_agree` / `children_states_agree`），`compose` 在 typed 参与者之间自动检查 |
| 单位（米/毫米） | ❌ 量纲相同，不检查 |
| 输出缩放、坐标系、符号 | ❌ 语义问题 |
