...
| 16 | **运行中安装执行路径被强制拒绝**（2026-09-26，评审 D 下半的收尾）：`update()` 用 RAII 计数器标记"周期在飞"，`control_loop_busy()` 公开可查；周期在飞时 `set_two_phase_execution(true)` 与 `set_staged_execution_group(...)` **返回 ERROR 且不发布任何 generation**（因为准入判定对着**独立发布**的控制器列表）；`set_two_phase_execution(false)` 与 `clear_staged_execution_group()` **始终允许**（在飞周期持有自己的 generation，会用它开始时的状态跑完）。用例用一个 `update()` 阻塞在条件变量上的控制器把"周期在飞"变成确定状态，见 `test_runtime_reconfiguration.cpp`（3 用例）与 `REVIEW_REQUIREMENTS_RESPONSE_2026-09-24.md` §D.4 |
| 15 | **激活整组回滚（可选，2026-09-26 两次增强）**：`set_atomic_activation(true)`（参数 `atomic_activation`，**默认 false**）。上游把一次 switch 的 activate 集合尽力而为地逐个激活，失败者跳过、成功者保持激活——上游自己的 spawner 测试依赖这一点，所以默认不动。开启后，本次 switch 造成的副作用被逐项撤销：① 本次从 INACTIVE 激活的控制器**逆序** `deactivate()` + `release_interfaces()`（评审 P1-1），其 reference interface 置回 unavailable，并对其 command interface 执行 `prepare/perform_command_mode_switch({}, ifaces)` **把硬件 command mode 换回去**；② **为切 chained mode 而被重启的既有控制器**（上游 `set_chained_mode()` 只允许非 ACTIVE 时调用，所以它被先停后启）被**恢复为 ACTIVE** 并回到切换前的 chained mode，重新走普通激活路径（重新 claim 接口、重新发布 reference）——否则一次失败的 switch 会顺带停掉一个与失败无关的、本来在运行的控制器；③ 本次做过的**所有** chained-mode 切换都回退，包括没有被激活的 following controller。判据来自 `switch_controller()` 在改写请求列表前抓的**切换前快照** `pre_switch_state_`（名字/是否 ACTIVE/是否 chained，经 `do_switch` 的 release-acquire 发布给实时线程），`ActivationOutcome::activated` 因此是每控制器一条记录（名字 + 本次 claim 的 command interface + 是否 chainable），硬件换回**排除**仍然 ACTIVE 的 restart 控制器。每一步失败单独记为 rollback failure（`ActivationOutcome::rollback_failed`），不再与原始 activation failure 混在一起。**实测证据**：mock 硬件模式计数器（`joint1/position`）在回滚后为基线 **+202**（关掉硬件那一步的探针为 **+101**）；关掉 restart 恢复逻辑的探针会让 `child` 停在 INACTIVE（`'\x2'` vs 期望 `'\x3'`）。作用域：**本次 switch** 触碰到的控制器；用户显式请求的 deactivate 不撤销（上游顺序如此）。见 `test_atomic_activation.cpp`（7 用例）与 `REVIEW_COMPILETIME_LATEST_RESPONSE_2026-09-26.md` §1 |
...
| 17 | **`manifest_problem()` 的父链检查已补**（2026-09-26 评审 P2-1）：新增对每个节点的**有界父链上溯**（≤ N 步），走不到根即 `"the parent chains must not contain a cycle"`。注意两种形状不同：**纯环** `a→b→c→a` 本来就被"恰好一个根"拦住；真正的漏洞是**链进入环**（根 `r` + `a→b, b→c, c→b`）——恰好一个根、父节点都存在、无自父，但 `a` 永远到不了根。用例三种形状都钉住，并在 `static_assert` 中使用（证明仍可用于编译期）。手工构造 `StaticManifest` 的调用者现在会被完整拒绝 |
| 18 | **静态树的部分激活已成为显式策略**（2026-09-26 评审 P1-2）：安装 staged group **要求** `atomic_activation == true`（`set_staged_execution_group()` 在关闭时返回 ERROR；已安装时 `set_atomic_activation(false)` 被拒），因为只有回滚能保证**失败的多控制器 switch** 不把整棵树留在半激活。逐条激活造成的部分成员仍然合法，此时执行组**完全惰性**（`StagedStatus::inactive`：不调用任何成员、不提交命令、ACTIVE 成员保持 claim），并在 `switch_controller()`（非实时线程）打印点名未激活成员的 WARN。用例：`installing_a_staged_group_requires_atomic_activation`、`a_partial_membership_is_inert`（实测：部分状态下 5 个周期后 `state_calls`/`commit_calls`/`legacy_update_calls` 全不变，补齐后 3 周期恰好 +3）。**构造部分状态只能靠逐个激活**：上游拒绝在父节点仍激活时停用链式子节点（见上表 #10），最初用"停用成员"构造的用例被实测否定后重写 |
| 19 | **compiled-in registry 的注册时机已受约束**（2026-09-26 评审 P2-2）：`StaticControllerRegistry` 新增 `freeze()`/`frozen()`，封印后 `add()`/`add_factory()` 抛 `std::logic_error`；`load_controller()` 在**第一次查表前**封印，`add_controller_impl()` 也封印；`set_static_controller_registry()` 返回 `bool`，已封印时**拒绝替换**且不改动已安装者；`register_static_controller_type()` 返回 `bool`，封印后拒绝（重复/空类型名仍抛 `std::invalid_argument`）。封印单向：卸载不会重新打开。**仍然不能声称**：这不是"并发注册安全"，而是"封印后只读"——封印之前的并发写仍不在支持范围内 |
| 20 | **参数化 factory 也能携带编译期描述**（2026-09-26 评审 P2-3）：新增 `StaticControllerRegistry::ManifestDescriptor` 与 `add_factory(type, factory, descriptor)`；`add_factory<ControllerT>(type, factory)` 从 `ControllerT::manifest` 派生（无 manifest 则 `static_assert` 并提示改用 descriptor）。原 `add_factory(type, factory)` 保留且 `has_manifest == false`（factory 本身不携带描述，不替调用者编造）。用例 `a_parameterised_factory_can_carry_its_manifest` |
| 21 | **TSan 脚本不再把"没有 race 日志"当作成功**（2026-09-26 评审 P2-4）：`run_tsan_real_manager.sh` 分两个 verdict——功能 verdict 要求进程有 `[==========] N tests ... ran.` 行、两个预期 suite 出现、`[ RUN ]` 数 ≥ 20，否则 `DID-NOT-RUN` 并以退出码 3 结束；race verdict 仍是 `data race` 计数；TSan 下计时敏感用例的失败单独列出。退出码：0 = race 干净且套件确实跑完，1 = 有 race，3 = 没跑完。**脚本逻辑改动本身只做了语法检查**（完整 TSan 需重建 `build_tsan`，为腾磁盘已删除；脚本会在二进制缺失时自动重建） |
