...
| 15 | **激活整组回滚（可选）**：`set_atomic_activation(true)`（参数 `atomic_activation`，**默认 false**）。上游把一次 switch 的 activate 集合尽力而为地逐个激活，失败者跳过、成功者保持激活——上游自己的 spawner 测试依赖这一点，所以默认不动。开启后，本次 switch 激活成功的控制器在任一成员失败时被撤销：**逆序** `deactivate()` + `release_interfaces()`（2026-09-26 评审 P1-1）、本次发布的 reference interface 置回 unavailable、并对其 command interface 执行 `prepare/perform_command_mode_switch({}, ifaces)` **把硬件 command mode 也换回去**；每一步失败单独记为 rollback failure（`ActivationOutcome::rollback_failed`），不再与原始 activation failure 混在一起。实测证据：mock 硬件模式计数器（`joint1/position`）在回滚后为基线 **+202**（关掉硬件那一步的探针为 **+101**）。作用域仍仅限**本次 switch 激活的**控制器；此前已激活的不动。**已知限制**：为切 chained mode 而被本次 switch 先停后启的既有控制器，回滚后停在 INACTIVE 而非原 ACTIVE。见 `test_atomic_activation.cpp`（6 用例）与 `REVIEW_COMPILETIME_LATEST_RESPONSE_2026-09-26.md` §1 |
