...
| 7 | 模式/成员/计划的发布**不是一个 generation**：标志与成员快照各自原子、启用/禁用顺序已固定，但它们与控制器列表、staged group 仍独立发布 ⇒ **首版要求"控制循环停止时配置"**（写在 `set_two_phase_execution` 的注释里）；完整 generation 协议未实现（评审 D）。**2026-09-26：** 真实 manager 的 TSan 已补（见 §12.4 #13），并因此把上游握手字段（`switch_params_` 的标志、控制器列表索引）改成原子 + release/acquire |
