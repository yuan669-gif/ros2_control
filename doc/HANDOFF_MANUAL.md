...
> 显式两阶段契约（同一线性顺序、状态阶段正向、命令阶段反向）可以修复它，且零额外存储。

**下一个会话应该做的第一件事**（按优先级）：

1. 读第 12 节的 4 份文档，然后**不要立刻写代码**，先向用户确认下面这个选择：

   | 选项 | 含义 | 风险 |
   |---|---|---|
   | **A. 先把论文写出来**（推荐） | 已有证据足够支撑一篇「不可能性 + 代价量化 + 真实系统验证」的论文；先立骨架，再补实验 | 无，纯增益 |
   | B. 补一个性能侧正结果 | 目前**所有性能指标都是负结果或不显著**（不更快、轨迹误差无显著收益）。需要构造高带宽/深层次场景才可能有正结果 | 可能又得到负结果，且需要新的仿真工作 |
   | C. 补 Jazzy/Rolling 数据通路复现 | 本机装不了（Ubuntu 22.04、无 Docker），需用户提供环境 | 阻塞 |

2. 无论选哪个，**先读第 11 节的「不能声称」清单**，避免把已有的诚实结论写反。
...
关键洞察：两趟**不需要两份拓扑信息**。后序（postorder）的反转天然就是「父先于后代」的合法顺序，
所以只用一份线性化、反向遍历即可，存储是 `O(|V|)` 而非两份数组。
...
即：单趟必然有一个方向陈旧且正比于深度；两趟两个方向都同周期，且**零额外存储**。
...
void set_staged_execution_group(...);                                   // 设置/清除
void set_two_phase_execution(bool enabled);                             // line 151
bool two_phase_execution() const;                                       // line 154
```

私有成员：`staged_group_`(525)、`no_two_phase`(529)、`struct TwoPhaseEntry`(530)、
`two_phase_enabled_`(535)、`two_phase_entries_dirty_`(536)、`two_phase_entries_`(537)。
...
2. **实时路径禁止获取 `controllers_lock_`（会导致死锁）**：
   ...
   - 正确做法（已实现）：非实时线程（`set_two_phase_execution`）可以加锁；
...
| `doc/WHY_NO_SPEEDUP.md` | ⚠ **尚不存在，待写**（见第 6 节 P0） |
