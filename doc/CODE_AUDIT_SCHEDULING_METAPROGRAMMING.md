...
| `hierarchical_control/test/test_typed_ports.cpp` | 适配新模型；新增 3 个负向 + 1 个正向校验用例；`wheel_ports` 示例改为四组 |
...
| 父子**状态边**没有静态成对检查 | 父不声明"我要读子节点的哪些状态"，该边由内核按 `parents` 拓扑决定；`ForChildren` 只覆盖 reference 方向 |
