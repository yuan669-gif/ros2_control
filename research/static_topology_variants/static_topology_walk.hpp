...
///
/// The ancestry is a QUERY, not a stored member (2026-09-24). An earlier revision materialised
/// `static constexpr std::array<std::string_view, depth> ancestry` in every node by copying the
/// parent's array, which is O(d) work per level and therefore O(d^2) for a chain of depth d. It
/// showed up directly in the measurement: 10.55 ms per instantiated node at chain depth 64 versus
/// 1.12 ms for a wide fan-out (see doc/COMPILE_COST.md). Walking the parent chain instead is O(d)
/// per query with O(1) per step, and nothing outside this header ever read the array.
///
/// `ancestry_of<NodeT>()` still produces the root-first, self-last array on demand, for callers that
/// want the array shape.
...
/// The name of the node `i` levels above `NodeT` (0 = `NodeT` itself, 1 = its parent, ...).
template <typename NodeT>
constexpr std::string_view ancestor_name(std::size_t up)
{
  if (up == 0) {return view_of(name_value_v<typename NodeT::name_type>);}
  if constexpr (std::is_void_v<typename NodeT::parent_type>)
  {
    return std::string_view{};  // out of range for a root; callers bound by `depth`
  }
  else
  {
    return ancestor_name<typename NodeT::parent_type>(up - 1);
  }
}

/// The root-first, self-last ancestry of a node type, materialised on demand.
template <typename NodeT>
constexpr std::array<std::string_view, NodeT::depth> ancestry_of()
{
  std::array<std::string_view, NodeT::depth> out{};
  for (std::size_t i = 0; i < NodeT::depth; ++i)
  {
    out[i] = ancestor_name<NodeT>(NodeT::depth - 1 - i);
  }
  return out;
}
...
/**
 * Walks the parent chain and returns as soon as it finds a match, so the common rejections (an
 * immediate self-loop or reuse of the direct parent) cost one step.
 */
...
constexpr bool ancestry_contains()
{
  ...
  if (view_of(name_value_v<typename NodeT::name_type>) == candidate) {return true;}
  if constexpr (std::is_void_v<typename NodeT::parent_type>)
  {
    return false;
  }
  ...
  {
    return ancestry_contains<typename NodeT::parent_type, NameT>();
  }
}
